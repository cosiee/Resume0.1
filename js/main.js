import { getDomElements, prioritizedImages, thumbnailImages } from "./universalConst.js";
import { preloadImages, setRandomBackgroundWithTransition,
  mountainSkyAni, getRandomImage, preloadThumbnailImages, animateMeAndWiggles, 
  getComputedStyleValue, getThumbWidthWithMargin, getScaleValue, updateMeElement, 
  animateThumbs, debounce, formControl, getScrollbarWidth, updateModalDimensions,
  updateWIPDimensions, collectThumbs, updateDimensionsNoMargins, getThumbMargin,
  getThumbWidthWithoutMargin, spaceoutThumbs, updateDimensions, updateEndBottomY,
  updateEndTopY, autoScroll, autoScrollNow, hideForm, showThumbs, hideWip, 
  showWip, showStatementContact, showForm} from "./universalUtils.js";


const thresholdScale = 1.5; //set scale value, to allow for the crossing-fading effect SEE/ME
let domElements; // Declare globally for use outside of DOMContentLoaded

document.addEventListener("DOMContentLoaded", function () {
  const domElements = getDomElements(); // Initialize after DOM is fully loaded
  console.log("domElements: ", domElements); // Debug log to check domElements

  if (domElements.thumbnailsContainer) {
    domElements.thumbnailsContainer.style.opacity = 0;
  } else {
...
if (domElements.thumbnailsContainer) {
  domElements.thumbnailsContainer.style.opacity = 1; // Safely access the property from domElements
} else {
  console.error("#thumbnails element not found.");
}
...
const thumbWidthWithMargin = getThumbWidthWithMargin(domElements.thumbElement);
console.log("Thumb width with margin:", thumbWidthWithMargin);

landscapeMediaQuery.addEventListener("change", updateEndTopY, updateEndBottomY);


animateMeAndWiggles(domElements.meElement, domElements.meShaker, domElements.seeText, domElements.down);
