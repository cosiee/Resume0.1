...
// Single public key for all forms
(function () {
    emailjs.init("U2lLF_M1IbCyOp1cU");
})();

// Wait a tiny bit for navbar.js to finish
setTimeout(() => {
    ...
    // Detect if we're on nailedIt.html
    const currentPath = window.location.pathname.toLowerCase();
    const isNailedItPage = currentPath.includes('nailedit.html') || currentPath.endsWith('/nailedit');

    // Choose the correct service & template IDs based on page
    let serviceId, templateId;
    if (isNailedItPage) {
        serviceId  = 'service_fhvgxq4';     // ← your nailedIt service ID
        templateId = 'template_9gc3y2e';    // ← your nailedIt template ID
    } else {
        serviceId  = 'service_z1xs6lj';   // ← replace with actual second service ID
        templateId = 'template_i7uy2gp';  // ← replace with actual second template ID
    }

    // Base modal structure with dynamic form placeholder
    shadow.innerHTML = `
        ...
        <style>
            ...
            .glass-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); z-index: 998; opacity: 0; pointer-events: none; transition: opacity 0.3s ease; }
            .glass-contact-modal:not(.contact-form-hidden) ~ .glass-overlay { opacity: 1; pointer-events: auto; }
            .glass-contact-modal { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%); z-index: 1000; width: 500px; max-width: 90vw; max-height: 90vh; padding: 40px; border: 1px solid rgba(255,255,255,0.2); border-radius: 20px; background: rgba(208,205,205,0.4); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); box-shadow: 0 8px 32px rgba(0,0,0,0.9); overflow-y: auto; box-sizing: border-box; }
            ...
            .glass-field input, .glass-field select, .glass-field textarea { width: 100%; padding: 10px; font-size: 1.6rem; background: rgba(255,255,255,0.85); border: 1px solid rgba(0,0,0,0.2); border-radius: 10px; color: #000; box-sizing: border-box; }
            .glass-field input:focus, .glass-field select:focus, .glass-field textarea:focus { background: #fff; box-shadow: 0 0 0 3px rgba(0,0,0,0.1); outline: none; }
            ...
            .glass-close-btn { position: absolute; top: 15px; right: 20px; background: transparent; border: none; font-size: 2.8rem; font-weight: bold; color: #fff; cursor: pointer; width: 50px; height: 50px; line-height: 45px; text-align: center; z-index: 1002; }
            .glass-close-btn:hover { color: #ccc; }
        </style>
    `;

    const modal = shadow.getElementById('glass-contact-modal');
    const formContainer = shadow.getElementById('dynamic-form-container');
    const status = shadow.getElementById('status');
    const closeBtn = shadow.querySelector('.glass-close-btn');
    const overlay = shadow.querySelector('.glass-overlay');

    // Inject correct form based on page
    if (isNailedItPage) {
        ...
        formContainer.innerHTML = `
            <form id="contactForm">
                <div class="glass-field"><label for="name">Name *</label><input type="text" id="name" required></div>
                <div class="glass-field"><label for="email">Email *</label><input type="email" id="email" required></div>
                <div class="glass-field"><label for="subject">Subject *</label><input type="text" id="subject" required></div>
                <div class="glass-field"><label for="phone">Telephone Number</label><input type="tel" id="phone"></div>
                <div class="glass-field"><label for="message">Message *</label><textarea id="message" rows="5" required></textarea></div>
                <div class="glass-submit"><button type="submit">Send Message</button></div>
            </form>
    ...
    // Submit handler – uses correct service + template
    form.addEventListener('submit', (e) => {
        ...
        const values = {
            ...
            subject: shadow.getElementById('subject')?.value || 'No subject',
            reason:  shadow.getElementById('reason')?.value  || 'Not specified',
    ...
    // Your existing closeModal function (the working one)
    const closeModal = () => {
    ...
    // Rest of your modal open/close listeners (unchanged)
...
// // Initialize EmailJS with the correct key based on page
// (function () {
//     const currentPath = window.location.pathname.toLowerCase();
//     const isNailedItPage = currentPath.includes('nailedit.html') || currentPath.endsWith('/nailedit');

//     const publicKey = isNailedItPage 
//         ? "BSXVEqtYjrl5zOaJX"               // nailedIt.html key
//         : "U2lLF_M1IbCyOp1cU";    // ← REPLACE with actual second key
...
//     // Determine page type again (for form selection)
//     const currentPath = window.location.pathname.toLowerCase();
//     const isNailedItPage = currentPath.includes('nailedit.html') || currentPath.endsWith('/nailedit');
...
//     // Submit handler – works for both forms
//     form.addEventListener('submit', (e) => {
//         e.preventDefault();
//         status.textContent = 'Sending...';
//         status.style.color = '#ffffff';

//         // Collect all possible fields safely
//         const values = {
//             name:    shadow.getElementById('name')?.value    || '',
//             email:   shadow.getElementById('email')?.value   || '',
//             phone:   shadow.getElementById('phone')?.value   || 'Not provided',
...
//         emailjs.send('service_fhvgxq4', 'template_9gc3y2e', values)
//             .then(() => {
//                 status.textContent = 'Message sent successfully! Thank you.';
//                 status.style.color = '#aaffaa';
//                 form.reset();
...
//     // Your existing closeModal function (the one that worked)
//     const closeModal = () => {
//         console.log('closeModal triggered');
//         modal.classList.add('contact-form-hidden');
...
// (function () {
//   emailjs.init("BSXVEqtYjrl5zOaJX"); // emailjs public key
// })();
...
//                     <div class="glass-contact-inner">
//                         <h2 class="glass-title">Contact Andrew at Nailed It</h2>
//                         <p class="glass-subtitle">Enter your details below, And I'll take it from there</p>
...
//     if (
//       window.pageDomUtils &&
//       typeof window.pageDomUtils.enablePageInteractions === "function"
//     ) {
//       console.log("Calling enablePageInteractions");
//       window.pageDomUtils.enablePageInteractions();
//     } else {
//       console.warn(
//         "pageDomUtils.enablePageInteractions not available – using fallback",
//       );
//       // Fallback: re-enable pointer events everywhere
//       document
//         .querySelectorAll(
//           "body, html, a, button, input, select, textarea, .navbar-toggler",
//         )
//         .forEach((el) => {
//           el.style.pointerEvents = "auto";
//           if (
...
//     if (
//       e.key === "Escape" &&
//       !modal.classList.contains("contact-form-hidden")
//     ) {
//       closeModal();
//     }
//   });
...
//     emailjs
//       .send("service_fhvgxq4", "template_9gc3y2e", templateParams)
//       .then(() => {
//         status.textContent = "Message sent successfully! Thank you.";
//         status.style.color = "#aaffaa";
//         form.reset();
